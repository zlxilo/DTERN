...
def intersect_and_union2(pred_label,
    ...
    if isinstance(pred_label, str):
        if '.npy' in pred_label:
            pred_label = np.load(pred_label)
        else:
            pred_label = Image.open(pred_label)
...
def total_intersect_and_union(results,
    ...
    total_area_intersect = np.zeros((num_classes, ), dtype=np.float_)
    total_area_union = np.zeros((num_classes, ), dtype=np.float_)
    total_area_pred_label = np.zeros((num_classes, ), dtype=np.float_)
    total_area_label = np.zeros((num_classes, ), dtype=np.float_)
    # added by guolei
