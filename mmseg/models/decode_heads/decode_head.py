...
class BaseDecodeHead_clips2(nn.Module, metaclass=ABCMeta):
    """Base class for BaseDecodeHead_clips.
    ...
    def __init__(self,
        ...
        super(BaseDecodeHead_clips2, self).__init__()
        self._init_inputs(in_channels, in_index, input_transform)
        self.channels = channels
        self.num_classes = num_classes
        self.dropout_ratio = dropout_ratio
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg,batch_size, num_clips):
        ...
        Returns:
            dict[str, Tensor]: a dictionary of loss components
        """
        seg_logits = self.forward(inputs,batch_size, num_clips,img_metas)
        losses = self.losses(seg_logits, gt_semantic_seg)
        return losses
    ...
    def forward_test(self, inputs, img_metas, test_cfg, batch_size=None, num_clips=None):
        ...
        Returns:
            Tensor: Output segmentation map.
        """
        seg_logits=self.forward(inputs, batch_size, num_clips,img_metas=img_metas)
        return seg_logits
    ...
    def losses(self, seg_logit, seg_label):
        ...
        if isinstance(seg_logit, tuple):
            assert self.cross_method == 'Cluster', "cluster_centers only support cluster method"
            seg_logit,cluster_centers=seg_logit
