...
class WindowAttention3d3(nn.Module):
    ...
    def __init__(self, dim, expand_size, window_size, focal_window, focal_level, num_heads, 
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH [36,8]
        ...
        if self.expand_size > 0 and focal_level > 0: #expand_size=3, focal_level=2
    ...
    def forward(self, x_all, mask_all=None, batch_size=None, num_clips=None):
        ...
        """
        x = x_all[0][0] # target frame

        B0, nH, nW, C = x.shape # ori [1,63,63,256]
        assert B0==batch_size
        qkv = self.qkv(x).reshape(batch_size, nH, nW, 3, C).permute(3, 0, 1, 2, 4).contiguous() #()
        q, k, v = qkv[0], qkv[1], qkv[2]  # (1, 63, 63, 256)

        # partition q maps windows_size:7
        ...
        # print("windows_size: ",q_windows.shape, k_windows.shape, v_windows.shape) # windows_size:  torch.Size([81, 8, 49, 32]) torch.Size([81, 8, 49, 32]) torch.Size([81, 8, 49, 32]) [B*np,nH,pp,C//n]

        # focal_level1
        if self.expand_size > 0 and self.focal_level > 0: #expand_size=3, focal_level=2
            ...
            k_rolled = torch.cat((k_tl_windows, k_tr_windows, k_bl_windows, k_br_windows), 1).transpose(1, 2) # [B*np,4*pp,nH,C//n] B=1 => [B*np,nH,4*pp,C//n] [81,196,8,32]
            v_rolled = torch.cat((v_tl_windows, v_tr_windows, v_bl_windows, v_br_windows), 1).transpose(1, 2) # [B*np,4*pp,nH,C//n] B=1 => [B*np,nH,4*pp,C//n] [81,196,8,32]
            #print("k_tl_windows.shape: ", k_tl_windows.shape) # torch.Size([81, 49, 8, 32])  [B*np,pp,nH,C//n] B=1

            #print("k_rolled.shape: ", k_rolled.shape) # [81, 8, 196, 32]
            ##print("v_rolled.shape: ", v_rolled.shape) # [81, 8, 196, 32]
            # mask out tokens in current window
            # import pdb; pdb.set_trace()
            k_rolled = k_rolled[:, :, self.valid_ind_rolled]
            v_rolled = v_rolled[:, :, self.valid_ind_rolled]
            #print("k_rolled.shape: ", k_rolled.shape) # k_rolled.shape:  torch.Size([81, 8, 132, 32])  196-132 = 64
            ##print("v_rolled.shape: ", v_rolled.shape) # [81, 8, 132, 32]
            k_rolled = torch.cat((k_windows, k_rolled), 2) #[81,8,132+49=181,32]
            v_rolled = torch.cat((v_windows, v_rolled), 2) #[81,8,181,32]
            # print("k_rolled.shape: ", k_rolled.shape) # [81, 8, 181, 32]
            # print("v_rolled.shape: ", v_rolled.shape) # [81, 8, 181, 32] [b*np,nH,pp_out(rolled+windows),C//nH] #level1
        else:
            k_rolled = k_windows
            v_rolled = v_windows 

        # 以上皆为focal transformer的处理
        if self.pool_method != "none" and self.focal_level > 1:
            k_pooled = []
            v_pooled = []

            # pooled target frame，得到准备结果 ，不需要这一层的pooled,和最近的时间是一致的，而target应该只需要 level1的结果
            ...
            # pooled others frame，将reference frame的结果加入到k_pooled和v_pooled中，区别与cat的是加上了target的 pool结果，并且包含了k_rolled结果，v_rolled结果，
            # 与focal transformer的不同之处在于只是加上了reference frame的pool结果做attention
            # 修改： 1. 去掉自身pooled的结果 2. 去掉rolled_k和rolled_v， 3. 都去掉只计算交叉注意力，
            # 实际上的计算，相较于CAT而言,有块内注意力，但是没有块间注意力，可以尝试添加块间的全局注意力，直接修改计算attention的shape即可
            # 当前torch.Size([b*nh*nw, num_head, patch_size*patch_size, sub_space_size]) torch.Size([b*nh*nw, num_head, all_patch_size, sub_space_size])
        # 修改为(bc,nh*nw,patch_size*patch_size) (bc,nh*nw,all_patch_size) 这里还计算不了，需要修改一下，因为target块的切分和reference块的切分不一样,pooled_size不一样，而且reference块的切分是不一样的,数目是累加关系
            ...
            for k in range(len(self.focal_l_clips)):
                ...
                x_window_pooled = x_all[k+1] # batch_size, nWh, nWw, C
                nWh, nWw = x_window_pooled.shape[1:3] 
                mask = x_window_pooled.new(nWh, nWw).fill_(1)
            ...
            #print("k_pooled",len(k_pooled), k_pooled[0].shape, k_pooled[1].shape, k_pooled[2].shape) # level2-4 没有了target
            #print("k_rolled",k_rolled.shape)
        #                k_pooled 4 torch.Size([81, 8, 25, 32]) torch.Size([81, 8, 49, 32]) torch.Size([81, 8, 25, 32]) torch.Size([81, 8, 9, 32])
                # k_rolled torch.Size([81, 8, 181, 32])
            # focal transformer 中不pooled自身
            k_all = torch.cat([k_rolled] + k_pooled, 2) #level1_target [81,8,181,32] level2_frame:torch.Size([81, 8, 49, 32]) level3_frame:torch.Size([81, 8, 25, 32]) level4_frame:torch.Size([81, 8, 9, 32]) #[b*np,nH,pp_out_levs,C//n]
            v_all = torch.cat([v_rolled] + v_pooled, 2)
            # kall:[81,8,181+49+25+9=289,32] [b*np,nH,pp_out_all,C//n]
        else:
            k_all = k_rolled
            v_all = v_rolled
        ...
        #print("attn_out:",q_windows.shape, k_all.shape) 
        
        # [81,8,49,32] @ [81,8,289,32] = [81,8,49,289] [b*np,nH,pp,pp_out_all(all levels)] 
        ...
        window_area_rolled = k_rolled.shape[2] #181 for level1
        #print("window_area_rolled: ", window_area_rolled,window_area_rolled) # 181
        ...
        attn[:, :, :window_area, :window_area] = attn[:, :, :window_area, :window_area] + relative_position_bias.unsqueeze(0) #for level1 

        # add relative position bias for patches inside a window
        if self.expand_size > 0 and self.focal_level > 0: #[49-181]
        ...
        # [81,8,49,289] [b*np,nH,pp,pp_out_all(all levels)]  @ [81,8,289,32] [b*np,nH,pp_out_all,C//n] = [81,8,49,32] [b*np,nH,pp,C//n]
        x = (attn @ v_all).transpose(1, 2).reshape(attn.shape[0], window_area, C) #[b*np,pp,C]
        x = self.proj(x)
        x = self.proj_drop(x)
        # print(x.shape)
...
class CffmTransformerBlock3d3(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, expand_size=0, shift_size=0,
        ...
        self.window_size_glo = self.window_size # 7
        ...
        if self.pool_method != "none":
            for k in range(self.focal_level-1):
                ...
                if self.pool_method == "fc":
                    self.pool_layers.append(nn.Linear(window_size_glo * window_size_glo, 1)) #49
            ...
            for k in range(len(focal_l_clips)): #[1,2,3]
                if focal_l_clips[k]>self.window_size:
                    ...
                    window_size_glo = math.floor(self.window_size_glo / (focal_l_clips[k])) #[7 , 3,  2]
                if self.pool_method == "fc":
                    self.pool_layers_clips.append(nn.Linear(window_size_glo * window_size_glo, 1)) #[49,9,4]
    ...
    def forward(self, x):
        ...
        shortcut = x
        x=x.reshape(B0*D0,H0,W0,C).reshape(B0*D0,H0*W0,C) #[bt, H*W, C]

        
        x = self.norm1(x)
        x = x.reshape(B0*D0, H0, W0, C) #[bt,h,w,c]
        # print("input cffmtransformerblock3d3: ", x.shape) # torch.Size([4, 60, 60, 256])
        # print("window_size", self.window_size) # 7
        ...
        #print("after pad: ", x.shape) #torch.Size([4, 63, 63, 256])
        ...
        #print("ori:x_windows_all(target frame): ", x_windows_all[0].shape)
        x_windows_all_clips=[]
        x_window_masks_all = [self.attn_mask]
        #print('x_window_masks_all',x_window_masks_all) #None no mask
        x_window_masks_all_clips=[]
        #print("all focal_level: ", self.focal_level) #2
        if self.focal_level > 1 and self.pool_method != "none": 
            # if we add coarser granularity and the pool method is not none

            # 保留了focal_transformer中的自注意力计算的部分，自身的pooled
            for k in range(self.focal_level-1):     # k=0
                window_size_glo = math.floor(self.window_size_glo / (2 ** k)) # 7
                pooled_h = math.ceil(H / self.window_size) * (2 ** k) # 9,采用window_size不重叠分割后的大小
                pooled_w = math.ceil(W / self.window_size) * (2 ** k) # 9
                H_pool = pooled_h * window_size_glo #pool 的总高度
                W_pool = pooled_w * window_size_glo #pool 的总宽度
                #print("k: ", k)
                #print("window_size_glo: ", window_size_glo)
                #print("pooled_h: ", pooled_h)
                #print("pooled_w: ", pooled_w)
                #print("H_pool: ", H_pool) # 63
                #print("W_pool: ", W_pool)
                x_level_k = shifted_x[:,-1] # target frame (b,h,w,c)
                #print("ori x_level_k: ", x_level_k.shape) # torch.Size([1, 63, 63, 256])
                ...
                #print("x_level_k: ", x_level_k.shape) #torch.Size([1, 63, 63, 256])
                ...
                #print("x_windows_noreshape: ", x_windows_noreshape.shape) #torch.Size([1, 9, 9, 7, 7, 256])
                #print("pool_layers:", len(self.pool_layers), k)
                if self.pool_method == "fc":
                    ...
                    x_windows_pooled = self.pool_layers[k](x_windows_noreshape).flatten(-2) # B0, nWh, nWw, C   #linear(window_size*window_size,1),将一个区域进行pool，进行区块的划分                         
                #print('x_windows_pooled',x_windows_pooled.shape) #torch.Size([1, 9, 9, 256])
                x_windows_all += [x_windows_pooled] 
                x_window_masks_all += [None]
            #print("x_windows_all: ", len(x_windows_all),x_windows_all[0].shape) # 2 torch.Size([1, 63, 63, 256])
            x_windows_all_clips += [x_windows_all]
            #print('x_windows_all_clips',len(x_windows_all_clips),len(x_windows_all_clips[0])) # 1 2
            #print("ori_in_put:",x_windows_all_clips[0][0].shape) # 1 2 torch.Size([1, 63, 63, 256])
            #print("pooled input:",x_windows_all_clips[0][1].shape) # 1 2 torch.Size([1, 9, 9, 256]) 这里是不是对target frame的pool部分
            x_window_masks_all_clips += [x_window_masks_all]

            #print("other clips")
            #print("focal_l_clips: ", self.focal_l_clips,"self.window_size: ", self.window_size) #focal_l_clips:  [1, 2, 3] self.window_size:  7
            # reference frame 的pooled
            for k in range(len(self.focal_l_clips)):     
        ...
        #4 torch.Size([1, 9, 9, 256]) torch.Size([1, 9, 9, 256]) torch.Size([1, 18, 18, 256]) torch.Size([1, 27, 27, 256]) #[b,nh,nw,c]
        # poooled target 和 pooled_frame_-9是一致的,这里有点矛盾
        # print('x_windows_all_clips',len(x_windows_all_clips),x_windows_all_clips[0][1].shape,x_windows_all_clips[1].shape,x_windows_all_clips[2].shape,x_windows_all_clips[3].shape) # 4 [[src,pooled_src],[pooled1],[pooled2],[pooled3]]
        # print("dd",d)

        attn_windows = self.attn(x_windows_all_clips, mask_all=x_window_masks_all_clips, batch_size=B0, num_clips=D0)  # #[b*np,pp,C]
        # print("attn_windows",attn_windows.shape) #attn_windows torch.Size([81, 49, 256])
        # 下面这里不是没改变吗
        attn_windows = attn_windows[:, :self.window_size ** 2] #[b*np,pp,C]
        # print("attn_windows_out",attn_windows.shape) #attn_windows_out torch.Size([81, 49, 256])
        # merge windows
        ...
        assert x.shape==shortcut.shape #[b,d,h,w,c] 前几帧是没更新的，只有后面的更新了的
